...
function LibraryView({ songs, onOpen, onExport, onDelete, onReturn, onExportBackup, onImportBackup, onShowCloudCode, onRestoreCloud, onSyncImported, isSyncing, cloudStatus, showReturn }: { songs: SavedSong[]; onOpen: (song: SavedSong) => void; onExport: (song: SavedSong) => void; onDelete: (id: string) => void; onReturn: () => void; onExportBackup: () => void; onImportBackup: (file: File) => void; onShowCloudCode: () => void; onRestoreCloud: () => void; onSyncImported: () => void; isSyncing: boolean; cloudStatus: string; showReturn: boolean }) {
  ...
  const groupedArtists = useMemo(() => {
    ...
    for (const song of filtered) {
      const name = song.artist || "אמן לא צוין";
      groups.set(name, [...(groups.get(name) ?? []), song]);
    }
  ...
  return <main className="library-page">
    <section className="library-hero">
      <div><p className="eyebrow"><LibraryBig size={14} /> הספרייה האישית</p><h1>בחר אמן.<br /><em>התחל לנגן.</em></h1><p>כל השירים נשמרים בטלפון וזמינים גם ללא חיבור.</p><div className="library-summary"><strong>{songs.length}</strong><span>{songs.length === 1 ? "שיר שמור" : "שירים שמורים"}</span><i /></div></div>
    ...
    <section className="library-controls" aria-label="חיפוש, מיון וסינון ספרייה">
      ...
      <details className="library-utilities"><summary>גיבוי וסנכרון</summary><div className="library-backup-actions"><button onClick={onExportBackup}><Download size={15} /> גיבוי לספרייה</button><label><ExternalLink size={15} /> שחזור מקובץ<input type="file" accept="application/json,.json" onChange={(event) => { const file = event.target.files?.[0]; if (file) onImportBackup(file); event.currentTarget.value = ""; }} /></label></div><div className="library-backup-actions"><button onClick={onShowCloudCode}><KeyRound size={15} /> קוד שחזור ענן</button><button onClick={onRestoreCloud}><CloudDownload size={15} /> שחזר מהענן</button><button onClick={onSyncImported} disabled={isSyncing}><CloudDownload size={15} /> {isSyncing ? "מסנכרן…" : "סנכרן 247 שירים"}</button></div><p className="library-backup-note">{cloudStatus} · שמור את קוד השחזור במקום פרטי כדי שתוכל להחזיר את הספרייה לטלפון חדש.</p></details>
    </section>
...
export default function Home() {
  ...
  const syncImportedLibrary = async () => {
    ...
    try {
      ...
      const merged = mergeCloudSongs(library, cloudSongs.map((song) => makeSavedSong(song)));
      const next = writeSongLibrary(window.localStorage, merged);
      const added = next.length - library.length;
      setLibrary(next);
      setCloudStatus(added > 0 ? `נוספו ${added} שירים מהספרייה בענן` : "הספרייה כבר מעודכנת");
      window.alert(added > 0 ? `נוספו ${added} שירים. יש לך עכשיו ${next.length} שירים.` : `הספרייה כבר מעודכנת עם ${next.length} שירים.`);
    } catch { setCloudStatus("הסנכרון נכשל — נסה שוב עם חיבור לאינטרנט"); window.alert("לא הצלחתי לסנכרן כרגע. בדוק את החיבור לאינטרנט ונסה שוב."); }
  ...
  return <div dir="rtl" className="app-shell">
    ...
    {screen === "library" ? <LibraryView songs={library} onOpen={openSavedSong} onExport={(song) => exportPdf(song)} onDelete={deleteSavedSong} onReturn={() => { if (savedSong) { window.history.pushState({ chordshiftScreen: "player" }, "", "#song"); setHistoryVersion((value) => value + 1); setScreen("player"); } else setScreen("player"); }} onExportBackup={exportLibraryBackup} onImportBackup={importLibraryBackup} onShowCloudCode={() => void showCloudRecoveryCode()} onRestoreCloud={() => void restoreFromCloud()} onSyncImported={() => void syncImportedLibrary()} isSyncing={isSyncing} cloudStatus={cloudStatus} showReturn={Boolean(savedSong)} /> : <main className={`workspace ${savedSong ? "workspace-song-only" : ""}`}>
